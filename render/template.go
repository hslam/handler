...
func (t *Tmpl)Execute(w http.ResponseWriter, r *http.Request,data interface{}) (int,error){
	if t.t==nil{
		return 0,errors.New("Must NewTemplate")
	}
	header.SetContentTypeUTF8(w,header.ContentTypeHTML)
...
func Execute(w http.ResponseWriter, r *http.Request, text string,data interface{}) (int,error){
	...
	return t.Execute(w,r,data)
}

func ExecuteTemplate(w http.ResponseWriter, r *http.Request, filename,name string,data interface{}) (int,error){
	t:=NewTmpl()
	err:=t.ParseFiles(filename)
	if err!=nil{
		return 0,err
	}
	return t.ExecuteTemplate(w,r,name,data)
}
